...
void ImageLabelDataLayer<Dtype>::SampleScale(cv::Mat *image, cv::Mat *label) {
  ...
  double scale = (static_cast <double>(rand())/static_cast <double>(RAND_MAX))*(data_param.max_scale()-data_param.min_scale() + 1) + data_param.min_scale();
